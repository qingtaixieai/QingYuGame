...
try{
 ...
 await fixture([[0,0],[1,0],[-1,0]]);n=await count('cancel');await mark(a,1,0);await mark(a,-1,0);state=await current();assert.equal(state.turnPoints,2);assert.equal(state.intents.length,1);assert.equal(key(state.intents[0]),'-1,0');assert.equal(await count('cancel'),n+1);await mark(a,-1,0);assert.equal((await current()).turnPoints,2);await turn(a,1);await call('/battle/attack',{battleId,q:0,r:1},a,400);assert.equal(key((await current()).intents[0]),'-1,0');passed('Changing an untriggered mark spends two more points and keeps only the new cell');
 await fixture([[0,0],[1,0],[-1,0]]);n=await count();await mark(a,1,0);await move(a,-1,1);state=await current();assert.equal(await count(),n+1);assert.equal(state.intents.length,0);assert.equal(state.turnPoints,3);await mark(a,-1,0);await move(a,-1,2);state=await current();assert.equal(await count(),n+2);assert.equal(state.intents.length,0);passed('Triggered mark can be armed and triggered again in the same turn');
